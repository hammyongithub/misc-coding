...
vector<User*> UserManagementGraph::mostFollowing() {
    vector<User*> result; // To store the users with the most followers
    if (this->userNodes.empty()) return result; // If there are no users, return empty vector

    int maxFollows = 0; // Variable to store the maximum number of followings

    // Iterate over the adjacency list to find the max number of followers
    for (size_t i = 0; i < this->network.size(); ++i) {
        int followingCount = this->network[i].size();
        if (followingCount > maxFollows) {
            maxFollows = followingCount;
            result.clear(); // Clear previous results as we have a new maximum
            result.push_back(this->userNodes[i]); // Add current user as a new max follower
        } else if (followingCount == maxFollows) {
            result.push_back(this->userNodes[i]); // Add current user to the result list if ties in max
        }
    }

    return result; // Return the list of users with the most followers
}


TVSeries* UserManagementGraph::followingMostWatchedSeries(User* userPtr) {
    // Check if the provided user pointer is NULL, and if so, return NULL.
    if (userPtr == NULL) return NULL;
    // Retrieve the index of the user in the network data structure.
    int userIndex = userNodePosition(userPtr);
    // If the user is not found (userNodePosition returns -1), return NULL.
    if (userIndex == -1) return NULL;

    // A vector to keep track of series and total episodes watched by followers.
    vector<pair<TVSeries*, int>> seriesWatched;
    // A vector to track which series are watched by which followers.
    vector<pair<TVSeries*, vector<User*>>> seriesUsers;
    // We use pair to track 2 related stats, for example, the seriesWatched vector will have a TVseries pointer and an int for each index of the vector.
    
    // Loop over all users that the specified user follows.
    for (User* follower : network[userIndex]) {
        // Retrieve the list of series that the follower has watched.
        auto& watchedSeries = follower->getWatchedSeries();
        // Retrieve the count of episodes watched for each series.
        auto& episodesWatched = follower->getEpisodesWatched();
        // Iterate over each series the follower has watched.
        for (size_t i = 0; i < watchedSeries.size(); i++) {
            bool found = false;
            // Check if the series is already in seriesWatched; if so, update the count of episodes watched.
            for (auto& sw : seriesWatched) {
                if (sw.first == watchedSeries[i]) {
                    sw.second += episodesWatched[i];
                    found = true;
                    break;
                }
            }
            // If the series wasn't found in seriesWatched, add it with its watched episode count.
            if (!found) {
                seriesWatched.push_back(make_pair(watchedSeries[i], episodesWatched[i]));
            }

            found = false;
            // Check if the series is already in seriesUsers; if so, add the follower to the user list if not already present.
            for (auto& su : seriesUsers) {
                if (su.first == watchedSeries[i]) {
                    if (find(su.second.begin(), su.second.end(), follower) == su.second.end()) {
                        su.second.push_back(follower);
                    }
                    found = true;
                    break;
                }
            }
            // If the series wasn't found in seriesUsers, add it with the current follower.
            if (!found) {
                seriesUsers.push_back(make_pair(watchedSeries[i], vector<User*>{follower}));
            }
        }
    }

    TVSeries* mostWatched = NULL;
    int maxEpisodes = 0;
    int maxUsers = 0;

    // Loop through the seriesWatched to find the series with the most episodes watched.
    for (size_t i = 0; i < seriesWatched.size(); i++) {
        TVSeries* series = seriesWatched[i].first;
        int count = seriesWatched[i].second;
        int usersCount = seriesUsers[i].second.size();

        // Select the series with the most episodes watched, with ties broken by the number of distinct watching users.
        // If there's still a tie, use the series title lexicographically less as a tiebreaker.
        if (count > maxEpisodes || (count == maxEpisodes && (usersCount > maxUsers || (usersCount == maxUsers && (mostWatched == NULL || series->getTitle() < mostWatched->getTitle()))))) {
            mostWatched = series;
            maxEpisodes = count;
            maxUsers = usersCount;
        }
    ...
    // Return the series that has the most total episode views.
    return mostWatched;
}
...
{
    if (userSrc == NULL || userDst == NULL) return -1;
    int srcIndex = userNodePosition(userSrc);
    int dstIndex = userNodePosition(userDst);
    if (srcIndex == -1 || dstIndex == -1) return -1;

    vector<int> distances(userNodes.size());
