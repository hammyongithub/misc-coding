...
priority_queue<TVSeries> UserManagement::queueTVSeries(list<TVSeries*> listTV, int min) {
    // Create a priority queue to store TV series that meet the minimum viewership criteria.
    priority_queue<TVSeries> result;

    // Retrieve a vector of User pointers representing all users.
    vector<User*> allUsers = getVectorUsers();

    // Iterate through each TV series pointer in the list provided.
    for (TVSeries* series : listTV) {
        int count = 0;  // Initialize a counter for the number of users who have watched the series.

        // Iterate through all users to count how many have watched the current series.
        for (User* user : allUsers) {
            // Retrieve the list of TV series that the current user has watched.
            const auto& watchedSeries = user->getWatchedSeries();

            // Check if the current series is in the user's watched series list.
            if (std::find_if(watchedSeries.begin(), watchedSeries.end(),
                [&series](const TVSeries* s) { return s == series; }) != watchedSeries.end()) {
                count++;  // Increment count if the series is found.
            }
        }

        // If the number of users who have watched the series meets or exceeds the minimum requirement, add the series to the result priority queue.
        if (count >= min) {
            result.push(*series);
        }
    }
...
list<User*> UserManagementTree::usersNotFan(NodeUser* node) {
    // Initialize an empty list of User pointers to store the result.
    list<User*> result;
    // Return the empty list if the node is null, handling the base case for recursion.
    if (node == nullptr) return result;

    // Initialize a counter to track the number of TV series that the user has not finished watching.
    int unfinishedCount = 0;
    // Retrieve a reference to a vector representing the number of episodes watched of each series by the user.
    const auto& episodesWatched = node->user->getEpisodesWatched();
    // Retrieve a reference to a vector of pointers to TVSeries objects representing the series watched by the user.
    const auto& watchedSeries = node->user->getWatchedSeries();

    // Loop through each series watched by the user.
    for (size_t i = 0; i < watchedSeries.size(); i++) {
        int totalEpisodes = 0;  // Initialize a counter for the total number of episodes in the current series.
        // Retrieve a vector representing the number of episodes per season for the current series.
        const auto& episodesPerSeason = watchedSeries[i]->getEpisodesPerSeason();
        // Sum up all episodes across all seasons of the current series.
        for (int episodes : episodesPerSeason) {
            totalEpisodes += episodes;
        }
        // Check if the number of episodes watched is less than the total episodes available.
        if (episodesWatched[i] < totalEpisodes) {
            unfinishedCount++;  // Increment the count of unfinished series.
        }
    }

    // If the user has more than two unfinished series, add the user's pointer to the result list.
    if (unfinishedCount > 2) {
        result.push_back(node->user);
    }

    // Recursively call usersNotFan for the left child of the current node and concatenate the results.
    list<User*> leftResult = usersNotFan(node->left);
    result.insert(result.end(), leftResult.begin(), leftResult.end());

    // Recursively call usersNotFan for the right child of the current node and concatenate the results.
    list<User*> rightResult = usersNotFan(node->right);
    result.insert(result.end(), rightResult.begin(), rightResult.end());
    
    return result;
}

vector<int> UserManagementTree::usersCategoryStatistics(NodeUser* node, string cat, int perc) {
    // Initializes a vector of three integers all set to zero to store the statistics:
    // [0] Total viewers that meet the episode percentage requirement,
    // [1] Viewers who've watched all episodes,
    // [2] Viewers who consider the genre as their favorite and meet the episode percentage.
    vector<int> stats(3, 0);

    // Base case for recursion: if the node is null, return the current statistics array.
    if (node == nullptr) return stats;

    // Lambda function to convert strings to lowercase to facilitate case-insensitive comparison.
    auto toLower = [](const string& data) {
        string newData;
        // Transforms each character to lowercase and appends to newData.
        std::transform(data.begin(), data.end(), std::back_inserter(newData), ::tolower);
        return newData;
    };

    // Normalize the input category to lowercase using the lambda function.
    string normalizedCat = toLower(cat);

    // Process the current node
    // Retrieve references to the lists of watched TV series and episodes watched by the user at this node.
    const auto& watchedSeries = node->user->getWatchedSeries();
    const auto& episodesWatched = node->user->getEpisodesWatched();

    // Iterate over each series watched by the user.
    for (size_t i = 0; i < watchedSeries.size(); i++) {
        string seriesGenre = toLower(watchedSeries[i]->getGenre());  // Normalize the series genre.
        // Check if the current series' genre matches the specified category.
        if (seriesGenre == normalizedCat) {
            int totalEpisodes = 0;  // Count the total episodes in the series.
            // Calculate the total number of episodes in the series.
            for (int episodes : watchedSeries[i]->getEpisodesPerSeason()) {
                totalEpisodes += episodes;
            }
            // Calculate the number of episodes that constitutes the given percentage of the total.
            int requiredEpisodes = (totalEpisodes * perc) / 100;

            // Check if the number of episodes watched is at least the required number.
            if (episodesWatched[i] >= requiredEpisodes) {
                stats[0]++;  // Increment the count of viewers meeting the percentage threshold.
                if (episodesWatched[i] == totalEpisodes) {
                    stats[1]++;  // Increment the count of viewers who have watched all episodes.
                }

                // Check if this series genre is among the user's favorites.
                bool isFavoriteGenre = std::find_if(node->user->getFavoriteGenres().begin(), node->user->getFavoriteGenres().end(),
                                                    [seriesGenre, &toLower](const string& genre) {
                                                        return toLower(genre) == seriesGenre;
                                                    }) != node->user->getFavoriteGenres().end();

                if (isFavoriteGenre) {
                    stats[2]++;  // Increment the count of viewers for whom this genre is a favorite.
                }
            }
        }
    }

    // Recurse for the left child and add the returned stats to the current stats.
    vector<int> leftStats = usersCategoryStatistics(node->left, cat, perc);
    vector<int> rightStats = usersCategoryStatistics(node->right, cat, perc);
    stats[0] += leftStats[0] + rightStats[0];
    stats[1] += leftStats[1] + rightStats[1];
    stats[2] += leftStats[2] + rightStats[2];

    // Return the combined statistics after processing both children.
    return stats;
}
