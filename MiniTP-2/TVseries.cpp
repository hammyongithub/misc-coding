...
list<TVSeries*> TVSeriesManagementList::seriesByCategory(string cat) const {
    // Create an empty list to store TV series objects
    list<TVSeries*> seriesList;

    // Iterate through each TV series pointer in the list of TV series
    for (TVSeries* series : listTVSeries) {
        // Check if the genre of the current TV series matches the given category
        if (series->getGenre() == cat) {
            // If it matches, add the pointer to the TV series to the seriesList
            seriesList.push_back(series);
        }
    }

    // Return the list of TV series belonging to the specified category
    return seriesList;
}
